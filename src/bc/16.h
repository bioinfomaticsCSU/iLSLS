...
bool sizeSatisfied(int c1, int c2, int sz, bool bvr1, bool bvr2, bool & strong, bool need2verifyParterner){ //c1 is rightedge, c2 is candidate
	strong = false;
	...
	if(sz >= 20){
		...
		vector<int> vsz;
		if(need2verifyParterner){
			if(bvr2){
				for(unordered_map<int, dbvspt >::iterator it = vmpsptl[c2].begin(); it!=vmpsptl[c2].end(); it++){
					if(bused[it->first])continue;
					if(it->second.vspt0.size())vsz.push_back(it->second.vspt0.size());
					if(it->second.vspt1.size())vsz.push_back(it->second.vspt1.size());
				}
				sort(vsz.begin(), vsz.end(), greater<int>());
			}else{
				for(unordered_map<int, dbvspt >::iterator it = vmpsptr[c2].begin(); it!=vmpsptr[c2].end(); it++){
					if(bused[it->first])continue;
					if(it->second.vspt0.size())vsz.push_back(it->second.vspt0.size());
					if(it->second.vspt1.size())vsz.push_back(it->second.vspt1.size());
				}
			...
			if(bvr1){
				for(unordered_map<int, dbvspt >::iterator it = vmpsptr[c1].begin(); it!=vmpsptr[c1].end(); it++){
					if(bused[it->first])continue;
					if(it->second.vspt0.size())vsz.push_back(it->second.vspt0.size());
					if(it->second.vspt1.size())vsz.push_back(it->second.vspt1.size());
				}
				...
				for(unordered_map<int, dbvspt >::iterator it = vmpsptl[c1].begin(); it!=vmpsptl[c1].end(); it++){
					if(bused[it->first])continue;
					if(it->second.vspt0.size())vsz.push_back(it->second.vspt0.size());
					if(it->second.vspt1.size())vsz.push_back(it->second.vspt1.size());
				}
		...
		if(vsz.size() == 1){
			...
			if(index == 0 || (index == 1 && (double)vsz[0]/vsz[1]<2))return true;
		}
	}
}
...
bool verifyzone(int start, int end, deque<spt> &vspt, target &t, int workctg, int origctg, bool bvr, bool need2verifyParterner){
	...
	for(; i >= 0; i--){
		if(vspt[i].posSelf - vspt[i+1].posSelf > 2*lib[FileNum].std){
			...
			if(sizeSatisfied(workctg, t.ctgno, vspt.size(), bvr, t.brevs, strg, need2verifyParterner)){
...
bool verifyStep1(deque<spt> &vspt, target &t, int workctg, int origctg, bool bvr, bool need2verifyParterner){
	...
	bool strg;
	if(!sizeSatisfied(workctg, t.ctgno, vspt.size(),bvr, t.brevs, strg, need2verifyParterner))return false;
