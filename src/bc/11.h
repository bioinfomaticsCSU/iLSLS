...
bool isPathOK(deque<target> &vtgt,int workctg, int rightedge){
	if(vtgt.size() == 0)return false;
	int prixlen = 0;
	int validpath = 0;
	...
	for(int i=0; i < (int) vtgt.size()-1; i++){
		...
		if(prixlen > lib[FileNum].MaxSpan - lib[FileNum].std){
			ofile<<"isPathOK step 1 failed "<<vtgt.size()<<' '<<rightedge<<endl;
			vtgt.clear();
			collectTargets2(workctg, vtgt, rightedge, scfdirs[workctg][scfdirs[workctg].size()-1], useAll, 0);
			for(int i=0; i < (int) vtgt.size()-1; i++){
				int noOfn = (vtgt[i].gapdis - prixlen);
				prixlen += (noOfn + OriVec[vtgt[i].ctgno].oristr.size());
				if(prixlen > lib[FileNum].MaxSpan - lib[FileNum].std){
					ofile<<"isPathOK step 2 failed "<<vtgt.size()<<' '<<rightedge<<endl;
					return false;
				}
			}
			return true;
...
bool verifyParterner(int workctg, int rightedge, bool brevs){
	...
	deque<target> vtgt;
	collectTargets(rightedge, vtgt, rightedge, !brevs, useAll, 0);
	...
	int startpos = 0;
	for(int i=0; i < (int) vtgt.size()-1; i++){
		startpos += vtgt[i].gapdis;
		if(umpPos.count(vtgt[i].ctgno)){
			if(umpPos[vtgt[i].ctgno] < 0 && vtgt[i].brevs || umpPos[vtgt[i].ctgno] > 0 && !vtgt[i].brevs){
				vtgt[i].gapdis *= vtgt[i].brevs?-1:1;
				if(abs(umpPos[vtgt[i].ctgno] - vtgt[i].gapdis) > lib[FileNum].std)return false;
			}else return false;
		}
	}
...
bool verifyzone(int start, int end, deque<spt> &vspt, target &t, int workctg, int origctg, bool bvr, bool need2verifyParterner){
	int &avg = t.partdif;
	int &gapdis = t.gapdis;
	// if(vspt[start].posSelf - vspt[end - 1].posSelf < 500){
		// ofile<<"gap may be too large"<<endl;
		// return false;
	// }
	...
	if(need2verifyParterner && !verifyParterner(origctg, t.ctgno, t.brevs)){
		ofile<<"verifyParterner failed"<<endl;
	}
	return true;
}
...
void collectTargets(int workctg, deque<target> &vtgt, int rightedge, bool brevs, bool buseAll, bool need2verifyParterner){
	need2verifyParterner = false;
	target t;
	int i = workctg;
	//right extension  unordered_map<int, vector<spt> >
	ofile<<"collectTargets: rightedge = "<<rightedge<<endl;
	if(rightedge == 469){
		rightedge = 469;
	}
	...
	if(!brevs){
		...
		for(unordered_map<int, dbvspt >::iterator it = vmpsptl[rightedge].begin(); it!=vmpsptl[rightedge].end(); it++){
			...
			if(it->second.vspt0.size() >= 20)ofile<<"candidate "<<it->first<<", brevs = false, surpports "<<it->second.vspt0.size()<<endl;
			t.ctgno = it->first;
			t.brevs = 0;
			...
			t.brevs = 1;
			if(it->second.vspt1.size() >= 20)ofile<<"candidate "<<it->first<<", brevs = true, surpports "<<it->second.vspt1.size()<<endl;
...
void collectTargets2(int workctg, deque<target> &vtgt, int rightedge, bool brevs, bool buseAll, bool need2verifyParterner){
	...
	int i = workctg;
	//right extension  unordered_map<int, vector<spt> >
	ofile<<"collectTargets2: rightedge = "<<rightedge<<endl;
	if(rightedge == 469){
		rightedge = 469;
	}
...
int decision(vector< deque<target> > &vdt){
	...
	for(int i = 0; i < vdt.size(); i++){
		if(lib[FileNum].aveSpan - vdt[i][0].gapdis != 0)vsc.push_back(vdt[i][0].pdspt->size() / (double)(lib[FileNum].aveSpan - vdt[i][0].gapdis));
		//if(lib[FileNum].aveSpan - vdt[i][0].gapdis != 0)vsc.push_back(1 / (double)(lib[FileNum].aveSpan - vdt[i][0].gapdis));
		else vsc.push_back(100.0);
	}
...
// deque<target> selectPath(deque<target> &vtgt,int workctg, int rightedge, int dir){
	...
	// for(int i=0; i < (int)vtgt.size()-1; i++){
		...
		// if(prixlen > lib[FileNum].MaxSpan - lib[FileNum].std){
			...
			// vtgt.clear();
			// collectTargets2(workctg, vtgt, rightedge, scfdirs[workctg][scfdirs[workctg].size()-1], notUseAll, 0);
