...
bool verifyStep1(deque<spt> &vspt, target &t, int workctg, int origctg, bool bvr){
	...
	for(; i >= 0; i--){
		...
		if(!binctg)continue;
		if(vspt[i].posSelf - vspt[i+1].posSelf > 2000*lib[FileNum].std){
			//find a zone, verify this zone
			if(end - i > 20){
				if(verifyzone(i+1, end+1, vspt, t, workctg, origctg, bvr))return true;
			}
			end = i;
		}
	}
