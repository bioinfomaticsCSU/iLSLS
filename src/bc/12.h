...
bool verifyzone(int start, int end, deque<spt> &vspt, target &t, int workctg, int origctg, bool bvr){
	int &avg = t.partdif;
	int &gapdis = t.gapdis;
...
bool verifyStep1(deque<spt> &vspt, target &t, int workctg, int origctg, bool bvr){
	...
	ofile<<endl;
	if(verifyzone(0, vspt.size(), vspt, t, workctg, origctg, bvr))return true;
	return false;
}

bool verifyzone2(int start, int end, deque<spt> &vspt, target &t, int workctg, int origctg, bool bvr){
	int &avg = t.partdif;
	int &gapdis = t.gapdis;
...
bool verifyStep2(deque<spt> &vspt, target &t, int workctg, int origctg, bool bvr){
	if(vspt.size()<20)return false;
	//divide zones
	...
	for(; i >= 0; i--){
		...
		if(vspt[i].posSelf - vspt[i+1].posSelf > 3*lib[FileNum].std){
			//find a zone, verify this zone
			if(end - i > 20){
				if(verifyzone2(i+1, end+1, vspt, t, workctg, origctg, bvr))return true;
			}
			end = i;
		}
	}
...
void collectTargets(int workctg, deque<target> &vtgt, int rightedge, bool brevs, bool buseAll){
	...
	if(!brevs){
		...
		for(unordered_map<int, dbvspt >::iterator it = vmpsptl[rightedge].begin(); it!=vmpsptl[rightedge].end(); it++){
			...
			t.ctgno = it->first;
			t.brevs = 0;
			if(verifyStep1(it->second.vspt0, t, rightedge, workctg, false)){
				t.brevs = 0;
			...
			if(verifyStep1(it->second.vspt1, t, rightedge, workctg, false)){
				t.brevs = 1;
		...
		for(unordered_map<int, dbvspt >::iterator it = vmpsptr[rightedge].begin(); it!=vmpsptr[rightedge].end(); it++){
			...
			t.ctgno = it->first;
			t.brevs = 1;
			if(verifyStep1(it->second.vspt0, t, rightedge, workctg, true)){
				t.brevs = 1;
				t.pdspt = &(it->second.vspt0);
				vtgt.push_back(t);
			}
			ofile<<"candidate "<<it->first<<", brevs = true, surpports "<<it->second.vspt1.size()<<endl;
			t.brevs = 0;
			if(verifyStep1(it->second.vspt1, t, rightedge, workctg, true )){
				t.brevs = 0;
				t.pdspt = &(it->second.vspt1);
				vtgt.push_back(t);
			}
...
void collectTargets2(int workctg, deque<target> &vtgt, int rightedge, bool brevs, bool buseAll){
	target t;
	int i = workctg;
	//right extension  unordered_map<int, vector<spt> >
	ofile<<"rightedge = "<<rightedge<<endl;
	...
	if(!brevs){
		...
		for(unordered_map<int, dbvspt >::iterator it = vmpsptl[rightedge].begin(); it!=vmpsptl[rightedge].end(); it++){
			...
			t.ctgno = it->first;
			t.brevs = 0;
			if(verifyStep2(it->second.vspt0, t, rightedge, workctg, false)){
				t.brevs = 0;
				t.pdspt = &(it->second.vspt0);
				vtgt.push_back(t);
			};
			...
			t.brevs = 1;
			ofile<<"candidate "<<it->first<<", brevs = true, surpports "<<it->second.vspt1.size()<<endl;
			if(verifyStep2(it->second.vspt1, t, rightedge, workctg, false)){
				t.brevs = 1;
				t.pdspt = &(it->second.vspt1);
				vtgt.push_back(t);
			};
		...
		for(unordered_map<int, dbvspt >::iterator it = vmpsptr[rightedge].begin(); it!=vmpsptr[rightedge].end(); it++){
			...
			t.ctgno = it->first;
			t.brevs = 1;
			if(verifyStep2(it->second.vspt0, t, rightedge, workctg, true)){
				t.brevs = 1;
				t.pdspt = &(it->second.vspt0);
				vtgt.push_back(t);
			}
			ofile<<"candidate "<<it->first<<", brevs = true, surpports "<<it->second.vspt1.size()<<endl;
			t.brevs = 0;
			if(verifyStep2(it->second.vspt1, t, rightedge, workctg, true )){
				t.brevs = 0;
				t.pdspt = &(it->second.vspt1);
				vtgt.push_back(t);
			}
...
// deque<target> selectPath(deque<target> &vtgt,int workctg, int rightedge, int dir){
	...
	// for(int i=0; i < vtgt.size()-1; i++){
		...
		// if(prixlen > lib[FileNum].MaxSpan - lib[FileNum].std){
			...
			// vtgt.clear();
			// collectTargets2(workctg, vtgt, rightedge, scfdirs[workctg][scfdirs[workctg].size()-1], notUseAll);
			// for(int i=0; i < vtgt.size()-1; i++){
				// int noOfn = (vtgt[i].gapdis - prixlen);
				// prixlen += (noOfn + OriVec[vtgt[i].ctgno].oristr.size());
				// if(prixlen > lib[FileNum].MaxSpan - lib[FileNum].std){
