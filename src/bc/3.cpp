...
bool verifyzone(int start, int end, vector<spt> &vspt, vector<spt> &vspt2, target &t, int workctg, bool bvr);
...
bool verifyzone(int start, int end, vector<spt> &vspt, vector<spt> &vspt2, target &t, int workctg, bool bvr){
	...
	zonemax = lib[FileNum].aveSpan - gapdis - 3*lib[FileNum].std;
	if(zonemax > OriVec[workctg].oristr.size())zonemax = OriVec[workctg].oristr.size() - lib[FileNum].std;
	//zonemin = zonemin + (zonemax - zonemin) * 0.25;
	//zonemax = zonemax - (zonemax - zonemin) * 0.25;
	...
	// if(zonemin2 < 0)zonemin2 = 0;
	// zonemax2 = lib[FileNum].aveSpan - gapdis - 3*lib[FileNum].std;
	// if(zonemax2 > OriVec[t.ctgno].oristr.size())zonemax2 = OriVec[t.ctgno].oristr.size() - 0;
	// zonemin2 = zonemin2 + (zonemax2 - zonemin2) * 0.25;
	// zonemax2 = zonemax2 - (zonemax2 - zonemin2) * 0.25;
	...
	// if(!binzone2){
		// ofile<<"verifyPartner failed 1"<<endl;
		// return false;
	// }
