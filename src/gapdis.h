...
bool verifyParterner(int workctg, int rightedge, bool brevs){
	...
	for(int i = 0; i < vtgt.size(); i++){
		if(vtgt[i].pdspt->size() > vtgt[maxid1].pdspt->size()) {
			maxid3 = maxid2;
			maxid2 = maxid1;
			maxid1 = i;
		}else if(vtgt[i].pdspt->size() > vtgt[maxid2].pdspt->size()) {
			maxid3 = maxid2;
			maxid2 = i;
		}else if(vtgt[i].pdspt->size() > vtgt[maxid1].pdspt->size()) {
			maxid1 = i;
		}
	}
	
	if(umpPos.count(vtgt[maxid1].ctgno)){
...
bool sizeSatisfied(int c1, int c2, int sz){ //c1 is rightedge, c2 is candidate
	//if( (OriVec[t.ctgno].oristr.size() > limitlen && OriVec[workctg].oristr.size() > limitlen) && vspt.size()<LimitSz)return false;
	//if( !(OriVec[t.ctgno].oristr.size() > limitlen && OriVec[workctg].oristr.size() > limitlen) && vspt.size()<LimitSz2)return false;
...
bool verifyzone(int start, int end, deque<spt> &vspt, target &t, int workctg, int origctg, bool bvr, bool need2verifyParterner){
	...
	for(; i >= 0; i--){
		if(vspt[i].posSelf - vspt[i+1].posSelf > 2*lib[FileNum].std){
			...
			if(end - i >= LimitSz || !(OriVec[t.ctgno].oristr.size() > limitlen && OriVec[workctg].oristr.size() > limitlen) && end - i >= LimitSz2){
...
bool verifyzone2(int start, int end, deque<spt> &vspt, target &t, int workctg, int origctg, bool bvr, bool need2verifyParterner){
	...
	zonemin = lib[FileNum].aveSpan - gapdis - OriVec[t.ctgno].oristr.size() + 1*lib[FileNum].std;
	if(zonemin < 0)zonemin = lib[FileNum].std;
	zonemax = lib[FileNum].aveSpan - gapdis - 1*lib[FileNum].std;
...
int zoneLength(target t, zoneLR &z){
	int res;
...
double scoreOnePath(deque<target> &dt){
	...
	for(int i = 0; i < dt.size(); i++){
		double f = 0.0;
		for(int j = 0; j < dt[i].pdspt->size(); j++){
			if((*dt[i].pdspt)[j].posSelf <= vz[i].left && (*dt[i].pdspt)[j].posSelf >= vz[i].right)
				f += exp( (-1)*(lib[FileNum].aveSpan - dt[i].gapdis - (*dt[i].pdspt)[j].partdif)*(lib[FileNum].aveSpan - dt[i].gapdis - (*dt[i].pdspt)[j].partdif)/(2*lib[FileNum].std*lib[FileNum].std) );
		}
		res += vzlen[i]*f;
	}
	res = res/total;
...
int decision(vector< deque<target> > &vdt, int rightedge, bool brvs){
	vector<double> vsc;
	...
	for(int i = 0; i < vsc.size(); i++){
		string str1, str2;
		if(brvs)str1 = OriVec[rightedge].revstr;
		else str1 = OriVec[rightedge].oristr;
		
		if(vdt[i][0].brevs)str2 = OriVec[vdt[i][0].ctgno].revstr;
		else str2 = OriVec[vdt[i][0].ctgno].oristr;
		
		if(isOverlap(str1, str2) != string::npos){
			cout<<"overlap "<<vdt[i][0].brevs<<endl;
			vsc[i] *= 1;
		}
		
		if(rightedge == 262 && vdt[i][0].ctgno == 60){
			cout<<"======================================="<<endl;
			cout<<brvs<<' '<<vdt[i][0].brevs<<endl;
		}
	}
	...
	//if(vsc[max] <= 0.253388 &&vsc[max] >= 0.233388)max = 0;
	cout<<endl;
	return max;
}
		...
		if(prixlen > lib[FileNum].MaxSpan - lib[FileNum].std){
			...
			for(int i=0; i < (int)vtgt.size()-1; i++){
				...
				if(prixlen > lib[FileNum].MaxSpan - lib[FileNum].std){
					...
					int mx = decision(vdt, rightedge, scfdirs[workctg][scfdirs[workctg].size()-1]);
					if(mx >= 0){
						cout<<rightedge<<" => ";
						for(int i = 0; i < vdt[mx].size(); i++)cout<<vdt[mx][i].ctgno<<' ';
						cout<<endl;
						for(int j = 0; j < vdt.size(); j++){
							for(int p = 0; p < vdt[j].size(); p++){
								cout<<vdt[j][p].ctgno<<' ';
							}
							cout<<endl;
						}
...
void insert(target &t, int r, bool isvtgt1){
	for(int p = 0; p < finalctg.size(); p++){
		int index = finalctg[p];
		for(int h= 0;h < scfs[index].size();h++){
			if(h%2 == 0){ // contig
				if(scfs[index][h] == t.ctgno){ // find the position
					//cout<<"find the position"<<endl;
					if(isvtgt1 && scfdirs[index][h]^t.brevs || !isvtgt1 && !scfdirs[index][h]^t.brevs) {//not the same direction
						// while(h < scfs[index].size() && t.gapdis > scfs[index][h+1]){
							// t.gapdis -= (scfs[index][h+1]+OriVec[scfs[index][h+2]].oristr.size());
							// if(t.gapdis < 0)t.gapdis = 0;
							// h+=2;
						// }
						if(h != scfs[index].size()-1){
							scfs[index][h+1] -= (t.gapdis + OriVec[t.ctgno].oristr.size());
							cout<<scfs[index][h+1]<<' '<<t.gapdis<<endl;
							scfdirs[index][h+1] -= (t.gapdis + OriVec[t.ctgno].oristr.size());
							if(scfs[index][h+1] < 0)scfs[index][h+1] = 0;
							if(scfdirs[index][h+1] < 0)scfdirs[index][h+1] = 0;
						}
						cout<<"zone: "<<scfs[index][h+1]<<' '<<scfs[index][h+2]<<endl;
						scfs[index].insert(scfs[index].begin() + h + 1, t.gapdis);
						scfs[index].insert(scfs[index].begin() + h + 2, r);
						scfdirs[index].insert(scfdirs[index].begin() + h + 1, t.gapdis);
						scfdirs[index].insert(scfdirs[index].begin() + h + 2, 1);
					}else{ // same direction
						// while(h >= 0 && t.gapdis > scfs[index][h-1]){
							// t.gapdis -= (scfs[index][h-1]+OriVec[scfs[index][h-2]].oristr.size());
							// h-=2;
						// }
						cout<<"zone: "<<scfs[index][h-2]<<' '<<scfs[index][h]<<endl;
						scfs[index].insert(scfs[index].begin() + h, r);
						scfs[index].insert(scfs[index].begin() + h + 1, t.gapdis);
						
						scfdirs[index].insert(scfdirs[index].begin() + h, 0);
						scfdirs[index].insert(scfdirs[index].begin() + h + 1, t.gapdis);
						...
						cout<<scfdirs[index][h-1]<<' '<<t.gapdis<<endl;
					}
					return;
				}
			};
